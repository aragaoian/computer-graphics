...
int main(int argc, char** argv) {
	...
	cube = criar_poligono(128, 128, CUBESIZE, CUBEFACES);
...
void keyboard(unsigned char key, int x, int y) {
	...
	case 27:
		...
		break;
	case 'o':
		escalar(cube, 0.9, 0.9);
		break;
	case 'p':
		escalar(cube, 1.1, 1.1);
		break;
	case 'k':
...
void keyboard_special(int key, int x, int y) {
	...
	switch (key) {
	case GLUT_KEY_DOWN:
		movimentar(cube, 10, (270 / 180.0) * 3.1416);
		break;

	case GLUT_KEY_UP:
		movimentar(cube, 10, (90 / 180.0) * 3.1416);
		break;

	case GLUT_KEY_RIGHT:
		movimentar(cube, 10, (0 / 180.0) * 3.1416);
		break;

	case GLUT_KEY_LEFT:
		movimentar(cube, 10, (180 / 180.0) * 3.1416);
		break;
...
void redraw(int value) {
	rotacionar(cube, velocidadeRotacao);
